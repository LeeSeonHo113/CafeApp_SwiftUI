...
let names4: Array<String> = [
    ...
    "망고요거트스무디",
    "딸기요거트스무"]
let prices4: Array<String> = ["HOT : 3,000\nICED : 3,500",
                               "ICED : 3,500",
                               "ICED : 3,500",
                               "HOT : 3,000\nICED : 3,500",
                               "HOT : 3,000\nICED : 3,000",
...
let images4: Array<String> = ["http://paikdabang.com/wp-content/uploads/2018/05/%EC%99%84%EC%A0%84%EC%B4%88%EC%BD%94.jpg",
...
let messages4: Array<String> = ["초코에 퐁당 빠지고 싶을때~!? 진짜~! 다크초콜렛이 올라간 완~전 진한 초코라떼",
...
struct Menu4View: View {
    ...
    @State private var toastMesssage = "..."
    
    var body: some View {
        var menuList: Array<Menu> = Array()
        var index = 0;
        for _ in names {
            let menu: Menu = Menu(name: names4[index], price: prices4[index], imageUrl: images4[index],
                                  message: messages4[index])
            menuList.append( menu )
            index += 1
        }
        return List(menuList) { menu in
            MenuRow(menu: menu, showToast: self.$showToast, toastMessage: self.$toastMesssage)
        }
        .navigationTitle("스페셜")
        .navigationBarTitleDisplayMode(.inline)
        .toast(isPresenting: self.$showToast) {
            AlertToast(displayMode: .banner(.pop), type: .regular, title: self.toastMesssage)
        }
    }
}
